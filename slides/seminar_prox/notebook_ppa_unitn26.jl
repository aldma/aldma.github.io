...
# ╔═╡ 5705a264-10df-11f1-960b-01644c0a607f
using ProximalOperators, Plots, PlutoUI

# ╔═╡ 8d01d966-77cc-4d48-bd5f-0ab5dbde716a
md"Create function ``f = \lambda \|\cdot\|_1``"

# ╔═╡ da035d9a-8c40-4123-8943-6be757ee5eb7
begin
	lambda = 2.0
	f = NormL1(lambda) # try also NormL0
end

# ╔═╡ c93d1466-9ee9-4e06-9869-89cd505d2370
md"Stepsize ``\gamma``"

# ╔═╡ 3de0b569-402e-4bf9-8de5-daeab420f29c
@bind γ Slider(0:0.01:1; default=0.5, show_value=true)

# ╔═╡ d3fa080e-5fcc-46dd-b46f-24fbdb6c48f9
Mf = MoreauEnvelope(f, γ)

# ╔═╡ 436cc41f-9bac-4677-a8f4-2ca246d405e8
begin
	npts = 1000
	xv = range(-2.0,2.0,npts)
	fx = zeros(npts)
	Mfx = zeros(npts)
	for i = 1:npts
		fx[i] = f([xv[i]])
...
# ╔═╡ bd2bc412-7d5a-4329-8eaa-43caca8a9410
md"The Moreau envelope is a lower bound on function ``f``. They share minima and minimizers."

# ╔═╡ 1b545bb7-6470-4e52-89cb-b00a48cc1950
begin
	x0 = randn(3)
	println("f = $(f(x0)), Mf = $(Mf(x0))")
end

# ╔═╡ 9594affc-f04c-4cf2-9484-053fba49b915
md"Here we run PPA: ``x_+ \in \operatorname{prox}_{\gamma f}(x)``"

# ╔═╡ 55891fbf-eafd-4b69-8c0b-db00880057ee
begin
	x = similar(x0)
	x .= x0
	println("iter  |  fx  |  x[1]  x[2]  x[3]")
	for i = 1:20
		z, fz = prox(f, x, γ)
		x .= z
		println("$(i) | $(fz) | $(x[1]) $(x[2]) $(x[3])")
	end
end
...
manifest_format = "2.0"
project_hash = "966451f2b854146895150d786e3077acd4e62f77"
